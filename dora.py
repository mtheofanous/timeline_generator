...
sidebar = st.sidebar.selectbox("**Select**", ["Event Details", "Styling Options"])
...
# Sidebar for styling options
    ...
    if options == "Bars":
        ...
        opacity = st.sidebar.slider("Bar opacity", 0.1, 1.0, 1.0)
        bar_width = st.sidebar.slider("Bar width", 0.1, 1.0, 0.2)
    ...
    if options == "Letters":
        st.sidebar.header("Letters")
        letter_color = st.sidebar.color_picker("Pick a color for letters", "#BBBBBB")
        letter_size = st.sidebar.slider("Letter size", 1, 40, 18)
        if st.sidebar.button("Reset"):
            letter_color = "#BBBBBB"
            letter_size = 18
    ...
    if options == "Grid":
        ...
        grid_width = st.sidebar.slider("Grid width", 0.1, 1.0, 0.2)
        grid_color = st.sidebar.color_picker("Pick a color for grid", "#020202")
        if st.sidebar.button("Reset"):
            grid_width = 0.1
            grid_color = "black"
    ...
    if options == "Timeline Size":
        ...
        width = st.sidebar.slider("Width", 100, 2000, 900)
        if st.sidebar.button("Reset"):
            height = 300
            width = 900
...
try:
    if st.session_state["events_df"].empty:
        ...
        event_timeline(st.session_state["events_df"], st.session_state["bar_color"], st.session_state["bar_width"], st.session_state["dot_color"],
    ...
    st.warning("Please fill in all the required fields includin.")
